...
public class Drive extends SubsystemBase {
	...
	private GenericEntry rbSetAngle = driveTab.addPersistent("RB Set Angle", 0)
			.withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min", -180, "max", 180, "center", 0))
	...
	private Drive(NavXGyro gyro) {
		...
		PIDController frontLeftPIDController = new PIDController(STEER_P, STEER_I, STEER_D);
		frontLeftPIDController.enableContinuousInput(0, ENCODER_COUNT_PER_ROTATION);
		frontLeft = new SwerveModule("FL", DriveConstants.FrontLeftSteer, DriveConstants.FrontLeftDrive, DriveConstants.FrontLeftAnalogInput, frontLeftPIDController, invertDrive,
				invertSteer);

		PIDController backLeftPIDController = new PIDController(STEER_P, STEER_I, STEER_D);
		backLeftPIDController.enableContinuousInput(0, ENCODER_COUNT_PER_ROTATION);
		backLeft = new SwerveModule("BL", DriveConstants.BackLeftSteer, DriveConstants.BackLeftDrive, DriveConstants.BackLeftAnalogInput, backLeftPIDController, invertDrive,
				invertSteer);

		PIDController frontRightPIDController = new PIDController(STEER_P, STEER_I, STEER_D);
		frontRightPIDController.enableContinuousInput(0, ENCODER_COUNT_PER_ROTATION);
		frontRight = new SwerveModule("FR", DriveConstants.FrontRightSteer, DriveConstants.FrontRightDrive, DriveConstants.FrontRightAnalogInput, frontRightPIDController, invertDrive,
				invertSteer);

		PIDController backRightPIDController = new PIDController(STEER_P, STEER_I, STEER_D);
		backRightPIDController.enableContinuousInput(0, ENCODER_COUNT_PER_ROTATION);
		backRight = new SwerveModule("BR", DriveConstants.BackRightSteer, DriveConstants.BackRightDrive, DriveConstants.BackRightAnalogInput, backRightPIDController, invertDrive,
				invertSteer);
